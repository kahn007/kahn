...
import { LayoutGrid, List, Trash2, CheckSquare, Square, Edit3, Copy, ChevronRight, Filter } from 'lucide-react'
...
export default function VariationManager() {
  const {
    ...
    removeVariation, updateVariation, setActiveTab,
  } = useAdStore()
  ...
  const handleDuplicate = (v) => {
    ...
    addVariations([{ ...v, id: crypto.randomUUID(), index: variations.length + 1 }])
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-white">
            Variations
            <span className="ml-2 text-lg text-gray-500">{variations.length}</span>
          </h2>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            onClick={() => selectedVariations.length === variations.length ? clearSelection() : selectAll()}
          >
          ...
          <div className="flex bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <button
              ...
              className={`p-2.5 transition-colors ${viewMode === 'grid' ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-white'}`}
            >
              <LayoutGrid size={14} />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-2.5 transition-colors ${viewMode === 'list' ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-white'}`}
            >
              <List size={14} />
            </button>
          </div>
          ...
          {selectedVariations.length > 0 && (
            ...
            >
              Push {selectedVariations.length} to Facebook
...
function VariationCard({ variation, brandContext, selected, onToggle, onDelete, onDuplicate, onEdit, onPreview, editing, onSave, onCancelEdit }) {
  ...
  return (
    <div
      className={`card relative cursor-pointer transition-all duration-200 hover:border-gray-600 ${
        selected ? 'border-brand-500/70 bg-brand-500/5' : ''
      }`}
    >
      {/* Select checkbox */}
      <button
      ...
      {/* Mini FB preview */}
      <div className="mt-4 mb-3" onClick={onPreview}>
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-800">
        ...
        <div className="flex gap-1">
          <button className="p-1.5 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-gray-300 transition-colors" onClick={onPreview} title="Preview">
            👁
          </button>
          <button className="p-1.5 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-gray-300 transition-colors" onClick={onEdit} title="Edit">
            <Edit3 size={13} />
          </button>
          <button className="p-1.5 rounded-lg hover:bg-gray-800 text-gray-500 hover:text-gray-300 transition-colors" onClick={onDuplicate} title="Duplicate">
            <Copy size={13} />
          </button>
          <button className="p-1.5 rounded-lg hover:bg-red-900/30 text-gray-500 hover:text-red-400 transition-colors" onClick={onDelete} title="Delete">
            <Trash2 size={13} />
          </button>
        </div>
      </div>
...
function ListRow({ variation, selected, onToggle, onDelete, onPreview }) {
  return (
    <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
      ...
      <div className="flex gap-1 flex-shrink-0">
        ...
        <button className="p-1.5 rounded-lg hover:bg-red-900/30 text-gray-500 hover:text-red-400" onClick={onDelete}>
          <Trash2 size={13} />
        </button>
      </div>
    </div>
