    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="page-subtitle">Track which ad variations are winning</p>
        </div>
      ...
      {insightsError && (
        <div className="warn-box">
          <p className="font-semibold mb-0.5">Metrics unavailable</p>
          <p>{insightsError}</p>
          <p className="mt-1 opacity-80">Make sure your Facebook token has <strong>ads_read</strong> permission and the token hasn't expired.</p>
        </div>
      )}
            ...
            {['spend', 'impressions', 'ctr', 'clicks'].map((key) => (
              <button
                ...
                className={`text-xs px-2.5 py-1 rounded-lg font-medium transition-all uppercase ${
                  sortBy === key ? 'bg-brand-500 text-white' : 'text-zinc-500 hover:text-white hover:bg-gray-800'
                }`}
              >
                {key}
              </button>
        ...
        {isLoadingAnalytics ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-zinc-500 uppercase tracking-wide border-b border-gray-800">
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        <span className={`font-bold ${isWinner ? 'text-yellow-400' : 'text-zinc-500'}`}>
                          {isWinner ? '🏆' : `#${i + 1}`}
                        </span>
                      </td>
                      <td className="px-4 py-3 max-w-[200px]">
                        <p className="text-white font-medium truncate">{ad.name}</p>
                        <p className="text-zinc-500 text-xs font-mono">{ad.id}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <span className={`badge ${ad.status === 'ACTIVE' ? 'bg-green-900/50 text-green-300' : 'bg-gray-800 text-zinc-400'}`}>
                          {ad.status}
                        </span>
                      </td>
            ...
            {sorted.length === 0 && (
              <div className="py-16 text-center px-6">
                ...
                {emptyReason === 'no_token' && (
                  <>
                    <p className="text-zinc-400 font-medium">Showing demo data</p>
                    <p className="text-gray-600 text-sm mt-1">Add your Facebook Access Token in Settings to see real ad performance.</p>
                  </>
                )}
                {emptyReason === 'no_ads' && (
                  <>
                    <p className="text-zinc-400 font-medium">No ads in your account yet</p>
                ...
                {!emptyReason && (
                  <>
                    <p className="text-zinc-400 font-medium">No ads found</p>
  ...
  return (
    <div className="card">
      <h3 className="font-semibold text-white text-sm mb-4 flex items-center gap-2">
        ...
        Winning Angle Tracker
        <span className="text-xs text-zinc-500 font-normal">avg CTR by copy angle</span>
      </h3>
