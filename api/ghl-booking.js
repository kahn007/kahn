// Vercel serverless function — handles Vapi GHL calendar tool calls
// Deployed at: /api/ghl-booking
// GHL credentials come via custom headers set by Vapi (configured in syncVapiAssistant)
...
export default async function handler(req, res) {
  ...
  const toolCalls = req.body?.message?.toolCallList || []
  if (toolCalls.length === 0) return res.status(200).json({ results: [] })
  ...
  const results = await Promise.all(toolCalls.map(async (tc) => {
    ...
    try {
      ...
      if (name === 'book_appointment') {
        ...
        // Book appointment (30-min default)
